...
router.get('/notifications', async (req, res) => {
  try{
    ...
    const notificationsBd = await UserSchema.findById(req.session.user._id);
    console.log(`NOTIFICATIONS BD ${notificationsBd}`);
    const notifications = new Notifications(notificationsBd);
    console.log(notificationsBd);
...
router.get('/logout', async (req, res) => {
  try{
    req.session.destroy();
    req.session = null;
    const redisRes = await client.delAsync(`sess:${req.sessionID}`);
    return res.end();
  }catch(err){
    console.log(err);
  }
